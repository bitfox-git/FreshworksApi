...
namespace Bitfox.Freshworks.Models
{
    public class BaseController : Network
    {
        ...
        public async Task<Result<TEntity>> Insert<TEntity>(TEntity body) where TEntity : IHasInsert<TEntity>, new()
        {
            ...
            return await PostApiRequest(endpoint, content);
        }
        ...
        public async Task<Result<TEntity>> AssignBulk<TEntity>(TEntity body) where TEntity : IHasAssignBulk
        {
            ...
            {
                throw new ArgumentException($"SelectedIDs is required for assigning a bulk.");
            }
            else if (body.OwnerID == null)
            {
                throw new ArgumentException($"OwnerID is required for assigning a bulk.");
            }
        ...
        public async Task<Result<TEntity>> Filters<TEntity>() where TEntity : IHasFilters
        {
